...
/**
 * Fluent builder for defining rollout variants in migrations.
 *
 * @example
 * ```php
 ...
 * ```
 * @example With partial rollout (no value for unassigned users)
 * ```php
 * $rollout->variant('feature_enabled', 10, true); // Only 10% get the feature
 * ```
 */
class RolloutDefinition
{
    ...
    /**
     * Add a control variant (usually the existing/default behavior).
     */
    public function control(float $weight, mixed $value = null): self
    {
        return $this->variant('control', $weight, $value);
    }

    /**
     * Add a treatment variant (usually the new/experimental behavior).
     */
    public function treatment(float $weight, mixed $value = null): self
    {
        return $this->variant('treatment', $weight, $value);
    }

    /**
     * Quick 50/50 A/B test setup.
     */
    public function fiftyFifty(mixed $controlValue, mixed $treatmentValue): self
    {
        return $this
            ->control(50, $controlValue)
            ->treatment(50, $treatmentValue);
    }

    /**
     * Gradual rollout with a single variant.
     */
    public function gradual(float $percentage, mixed $value): self
    {
        return $this->variant('enabled', $percentage, $value);
    }
